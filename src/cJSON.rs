...
fn add_item_to_object(
...

    {
        ...
        // If the object has no children, set this item as the first child
        if object_mut.child.is_none() {
            object_mut.child = Some(Rc::clone(&item));
        } else {
            // Otherwise, find the last child and append the new item
            let mut last = object_mut.child.as_ref().unwrap().clone();
            while last.borrow().next.is_some() {
                last = last.borrow().next.as_ref().unwrap().clone();
            }
